...
def main():
    ...
    allowed_dirs = [os.path.abspath("/app/applet/uploads"), os.path.abspath("uploads"), os.path.abspath("/tmp")]
    is_safe = any(abs_path.startswith(d) for d in allowed_dirs) or not abs_path.startswith("..")
    
    if not is_safe or not os.path.exists(abs_path):
        print("\n---JSON_RESULT_START---")
        print(json.dumps({"error": "Invalid or unsafe file path provided."}))
