...
def deterministic_split(input_docs: List[Any], train_ratio: float = 0.8, val_ratio: float = 0.1, seed: int = 42) -> Dict[str, List[Any]]:
    """Splits data into train/val/test sets deterministically by sorting on a stable key or value."""
    if not input_docs:
    ...
    return {
        ...
        'val': sorted_docs[train_end:val_end],
...
def generate_manifest(stats: Dict[str, Any], data_ids: List[Any] = None, dataset_config: Dict[str, Any] = None) -> str:
    ...
    manifest = {
        ...
        "validation_documents": stats.get('validation_documents', stats.get('val_documents', 0)),
        "test_documents": stats.get('test_documents', 0),
        "train_tokens": stats.get('train_tokens', 0),
        "validation_tokens": stats.get('validation_tokens', stats.get('val_tokens', 0)),
