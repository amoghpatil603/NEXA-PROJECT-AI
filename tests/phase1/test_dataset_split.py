...
class TestSplit(unittest.TestCase):
    ...
    def test_no_overlap(self):
        ids = list(range(50))
        res = deterministic_split(ids, train_ratio=0.8, val_ratio=0.1)
        overlap_train_val = set(res['train']) & set(res['val'])
        overlap_train_test = set(res['train']) & set(res['test'])
        overlap_val_test = set(res['val']) & set(res['test'])
        self.assertEqual(len(overlap_train_val), 0)
        self.assertEqual(len(overlap_train_test), 0)
        self.assertEqual(len(overlap_val_test), 0)
        self.assertEqual(len(res['train']) + len(res['val']) + len(res['test']), 50)
